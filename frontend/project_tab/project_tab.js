...
async function fetchProjectData() {
    try {
        ...
        if (result.status === 'success' && result.data && result.data.length > 0) {
            ...
            document.getElementById('projFilterName').innerHTML = '<option value="">No data</option>';
            document.getElementById('projFilterClient').innerHTML = '<option value="">No data</option>';
        }
    } catch (error) {
...
let _projNameMap = {};
let _projCodeMap = {}; // Changed from Client to Code
 
function _buildProjFilters() {
    const nameInput = document.getElementById('projFilterName');
    const codeInput = document.getElementById('projFilterCode'); // Changed from Client to Code
    const statusInput = document.getElementById('projStatus');
 
    const nameList = document.getElementById('projNameList');
    const codeList = document.getElementById('projCodeList'); // Changed from Client to Code
 
    _projNameMap = {};
    _projCodeMap = {};
 
    const nameOpts = [];
    const codeOpts = new Set();
    ...
    _projData.forEach((p, i) => {
        ...
        _projNameMap[name] = i;
        if (code && !_projCodeMap[code]) _projCodeMap[code] = i;
 
        nameOpts.push(name);
        if (code) codeOpts.add(code);
    });
 
    nameOpts.sort();
    const sortedCodes = Array.from(codeOpts).sort();
 
    nameList.innerHTML = nameOpts.map(v => `<option value="${_escP(v)}">`).join('');
    codeList.innerHTML = sortedCodes.map(v => `<option value="${_escP(v)}">`).join('');
 
    // Set first project values
    const firstProj = _projData[0];
    if (firstProj && nameInput) nameInput.value = firstProj.name || '';
    if (firstProj && codeInput) codeInput.value = firstProj.code || '';
    if (firstProj && statusInput) statusInput.value = _getStatusText(firstProj.status);
 
    // Name input → find and show that project, update Code
    nameInput.addEventListener('change', () => {
        const idx = _projNameMap[nameInput.value];
        if (idx !== undefined) {
            codeInput.value = _projData[idx].code || '';
            statusInput.value = _getStatusText(_projData[idx].status);
            _showProject(idx);
        }
    });
 
    // Code input → show first project with that Code, update name
    codeInput.addEventListener('change', () => {
        const idx = _projCodeMap[codeInput.value];
        if (idx !== undefined) {
            nameInput.value = _projData[idx].name || '';
            statusInput.value = _getStatusText(_projData[idx].status);
            _showProject(idx);
        }
    });
...
function _showProject(idx) {
    ...
    // 1. Sync the top banner filter inputs to match exactly what we are showing
    const nameInput = document.getElementById('projFilterName');
    const codeInput = document.getElementById('projFilterCode'); // Changed from Client to Code
    const statusInput = document.getElementById('projStatus');
    
    if (proj && nameInput) nameInput.value = proj.name || '';
    if (proj && codeInput) codeInput.value = proj.code || '';
    if (proj && statusInput) statusInput.value = _getStatusText(proj.status);
    ...
    const startKeys = projKeys.filter(k => k.toLowerCase().replace(/_/g, '').replace(/ /g, '') === 'projectstartdate');
    const endKeys = projKeys.filter(k => k.toLowerCase().replace(/_/g, '').replace(/ /g, '') === 'projectenddate');
    ...
    const allGrouped = new Set([...overviewFields, ...timelineFields, ...statusFields, ...budgetFields, ...managersFields]);
    // Skip duplicate/internal fields from Databricks SCD2 logic / legacy fields
    const skipFields = new Set(['id', 'customAttributes', 'status', 'startdate', 'enddate', 'modify', 'comments', 'accountmanager']);
    const otherFields = projKeys.filter(k => !allGrouped.has(k) && !skipFields.has(k.toLowerCase()) && !skipFields.has(k));
...
function _formatProjValue(key, val) {
    ...
    // Billing type
    if (key === 'billingType') {
        const map = { '0': 'Non-Billable', '1': 'Fixed Price', '2': 'T&M', '3': 'Retainer' };
        return map[s] || s;
    }
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            if (parsed.length === 0) return '<span style="color:var(--clr-text-muted)">None assigned</span>';
            const names = parsed.map(item => {
                if (typeof item === 'object') {
                    const first = item.firstName || item.first_name || '';
                    const last = item.lastName || item.last_name || '';
                    return item.name || item.displayName || `${first} ${last}`.trim() || JSON.stringify(item);
                }
                return String(item);
            });
            return names.map(n => `<span class="status-badge badge-active" style="margin:2px">${_escP(n)}</span>`).join(' ');
        }
    ...
    if (/^\d{4}-\d{2}-\d{2}/.test(s)) {
        ...
        if (s.startsWith('0001-') || s.startsWith('1900-')) return '<span style="color:var(--clr-text-muted)">Not set</span>';
    }
...
function switchProjTab(tab) {
    ...
    document.querySelectorAll('.proj-sub-panel').forEach(p => p.style.display = 'none');
    document.querySelectorAll('.proj-sub-tab').forEach(b => b.classList.remove('active'));
    ...
    try {
        ...
        if (mgrInput) {
            ...
            try {
                ...
                const currentMgr = (_projData[_currentProjIdx] || {}).accountmanager || '';
